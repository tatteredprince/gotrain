...
func binarySearchTestHelper(t *testing.T, seq []int, target, expect int) {
	...
	if got := binarySearch(seq, target); got != expect {
		t.Fatalf("got index %d but expected %d", got, expect)
	}
}
...
func TestBinarySearch(t *testing.T) {
	...
	t.Run("Fibonacci numbers", func(t *testing.T) {
		binarySearchTestHelper(t, []int{0, 1, 1, 2, 3, 5, 8, 13, 34}, 8, 6)
	})
	t.Run("Search even number among odds", func(t *testing.T) {
		binarySearchTestHelper(t, []int{2, 4, 6, 8, 10}, 3, -1)
	})
}
